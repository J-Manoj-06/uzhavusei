...
class CurrentBorrowedCard extends StatelessWidget {
  final Map<String, dynamic> transactionData;
  final VoidCallback onViewMyLibrary;

  const CurrentBorrowedCard({
    super.key,
  ...
  Widget build(BuildContext context) {
    final title = (transactionData['bookTitle'] ?? transactionData['listingTitle'] ?? transactionData['equipmentName'] ?? 'Borrowed Book').toString();
    final cover = (transactionData['bookCover'] ?? transactionData['listingImage'] ?? transactionData['imageUrl'] ?? '').toString();

    DateTime dueDate = DateTime.now().add(const Duration(days: 7));
    if (transactionData['dueDate'] is Timestamp) {
      dueDate = (transactionData['dueDate'] as Timestamp).toDate();
    } else if (transactionData['borrowUntil'] is Timestamp) {
      dueDate = (transactionData['borrowUntil'] as Timestamp).toDate();
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              isOverdue
                                  ? 'OVERDUE (${daysRemaining < 0 ? daysRemaining.abs() : 0}d)'
